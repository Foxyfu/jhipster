...
type RestOf<T extends IStudent | NewStudent> = Omit<T, 'studyStart' | 'studyEnd'> & {
...
export class StudentService {
  ...
  protected convertDateFromClient<T extends IStudent | NewStudent | PartialUpdateStudent>(student: T): RestOf<T> {
    return {
      ...student,
      studyStart: student.studyStart?.toJSON() ?? null,
      studyEnd: student.studyEnd?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restStudent: RestStudent): IStudent {
    return {
      ...restStudent,
      studyStart: restStudent.studyStart ? dayjs(restStudent.studyStart) : undefined,
      studyEnd: restStudent.studyEnd ? dayjs(restStudent.studyEnd) : undefined,
    };
  }
